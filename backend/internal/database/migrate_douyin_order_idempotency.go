...
// migrateDouyinOrderIdempotencyIndexes adds idempotency indexes for Douyin order ingestion.
// Safe to re-run (IF NOT EXISTS). Rollback: DROP INDEX IF EXISTS ux_orders_shop_platform_ext_order;
func migrateDouyinOrderIdempotencyIndexes(db *gorm.DB) error {
	if db == nil {
		return fmt.Errorf("migrate Douyin order idempotency: db is nil")
	}
	...
	for _, sql := range stmts {
		if err := db.Exec(sql).Error; err != nil {
			return fmt.Errorf("Douyin order idempotency index: %w", err)
		}
	}
	return nil
}
...
LIMIT 20`).Scan(&rows).Error
	if err != nil {
		return fmt.Errorf("Douyin duplicate order check failed: %w", err)
	}
	if len(rows) == 0 {
		return nil
	}
...
ORDER BY created_at ASC LIMIT 5`,
	...
	return fmt.Errorf("Douyin order idempotency migration blocked: found %d duplicate order groups (example shop=%s platform=%s external_order_id=%s count=%d sample_ids=%v); see docs/DOUYIN_DUPLICATE_DATA_REPAIR.md",
...
LIMIT 20`).Scan(&rows).Error
	if err != nil {
		return fmt.Errorf("Douyin duplicate order item check failed: %w", err)
	}
	if len(rows) == 0 {
		return nil
	}
...
ORDER BY created_at ASC LIMIT 5`, rows[0].OrderID, rows[0].ExternalItemID).Scan(&samples).Error
	...
	return fmt.Errorf("Douyin order idempotency migration blocked: found %d duplicate order_item groups (example order_id=%s external_item_id=%s count=%d sample_ids=%v); see docs/DOUYIN_DUPLICATE_DATA_REPAIR.md",
