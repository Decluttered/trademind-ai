...
const ErrCodeProductionBoundaryViolation = "P10_BOUNDARY_VIOLATION"

// ProductionCapabilityConfig describes fail-closed production capability controls.
type ProductionCapabilityConfig struct {
...
func loadProductionCapabilityConfig(appEnv string) ProductionCapabilityConfig {
	_ = appEnv
...
func (c ProductionCapabilityConfig) Validate(appEnv string) error {
	level := strings.ToUpper(strings.TrimSpace(c.CurrentAllowedLevel))
	if level == "" {
		level = "L0"
	}
	if level != "L0" && level != "L1" && level != "L3" {
		return fmt.Errorf("%s: allowed level must be L0, L1, or L3", ErrCodeProductionBoundaryViolation)
	}
	...
	if level == "L0" && anyRealCapability {
		return fmt.Errorf("%s: all real, mutation, worker, and retry capabilities must remain disabled at L0", ErrCodeProductionBoundaryViolation)
	}
	if level == "L1" && (c.RealProductDraftWriteEnabled || c.InventoryMutationEnabled || c.AutomaticRetryEnabled) {
		return fmt.Errorf("%s: write and retry capabilities are forbidden at L1", ErrCodeProductionBoundaryViolation)
	}
	if level == "L3" && (!c.RealProviderEnabled || !c.RealPlatformNetworkEnabled || !c.RealCredentialsEnabled || !c.RealProductDraftWriteEnabled || !c.BackgroundWorkerEnabled) {
		return fmt.Errorf("%s: L3 product draft writes require provider, network, credentials, draft-write, and worker controls", ErrCodeProductionBoundaryViolation)
	}
	if c.RealProductDraftWriteEnabled && c.AutomaticRetryEnabled {
		return fmt.Errorf("%s: automatic retry is forbidden for real product draft writes", ErrCodeProductionBoundaryViolation)
	}
	if c.OfflineOAuthEnabled && IsStagingOrProduction(appEnv) {
		return fmt.Errorf("%s: offline OAuth is development/test only", ErrCodeProductionBoundaryViolation)
	}
	if c.LocalCredentialKey != "" && IsStagingOrProduction(appEnv) {
		return fmt.Errorf("%s: local credential keys are forbidden in staging/production", ErrCodeProductionBoundaryViolation)
	}
	...
	if oauthStateTTL < time.Minute || oauthStateTTL > 30*time.Minute {
		return fmt.Errorf("%s: OAuth state TTL must be between 60 and 1800 seconds", ErrCodeProductionBoundaryViolation)
	}
	...
	if requestTimeout < time.Second || requestTimeout > 2*time.Minute || connectTimeout <= 0 || responseHeaderTimeout <= 0 {
		return fmt.Errorf("%s: provider timeout configuration is invalid", ErrCodeProductionBoundaryViolation)
	}
	...
	if maxResponseBytes < 64*1024 || maxResponseBytes > 10*1024*1024 || providerConcurrency < 1 || providerConcurrency > 10 || pageSize < 1 || pageSize > 100 || paginationLimit < 1 || paginationLimit > 100 {
		return fmt.Errorf("%s: provider capacity configuration exceeds the approved limits", ErrCodeProductionBoundaryViolation)
	}
	if c.DouyinAPIBaseURL != "" {
		...
		if err != nil || u.Scheme != "https" || !strings.EqualFold(u.Hostname(), "openapi-fxg.jinritemai.com") || (u.Port() != "" && u.Port() != "443") || u.User != nil || u.RawQuery != "" || u.Fragment != "" {
			return fmt.Errorf("%s: P10_DOUYIN_API_BASE_URL must be a trusted HTTPS URL without userinfo", ErrCodeProductionBoundaryViolation)
		}
	}
	for _, raw := range c.RedirectAllowlist {
		u, err := url.Parse(strings.TrimSpace(raw))
		if err != nil || u.Scheme == "" || u.Host == "" || u.Fragment != "" {
			return fmt.Errorf("%s: invalid OAuth redirect allowlist entry", ErrCodeProductionBoundaryViolation)
		}
	}
	return nil
}
