...
type inventoryPostgresBarrierProvider struct {
	base    InventoryProvider
	barrier <-chan struct{}
}

func (p inventoryPostgresBarrierProvider) Key() InventoryProviderKey { return p.base.Key() }
func (p inventoryPostgresBarrierProvider) Capabilities() InventoryProviderCapabilities {
	return p.base.Capabilities()
}
func (p inventoryPostgresBarrierProvider) FetchInventoryPage(ctx context.Context, req InventoryFetchRequest) (InventoryFetchPageResult, error) {
	select {
	case <-ctx.Done():
...
func TestInventoryPostgresSchemaForeignKeysPartialIndexesAndTimeContract(t *testing.T) {
...
func TestInventoryPostgresConcurrentDatabaseConstraintsAndIdempotency(t *testing.T) {
	ctx := context.Background()
	db := newInventoryPostgresTestDB(t)
	tenantID := inventoryPostgresTenantID()
	store, prod, sku := inventoryPostgresSeedShopAndSKU(t, db, tenantID, "constraint-concurrency")
	run := inventoryPostgresCreateRun(t, ctx, db, tenantID, store.ID, "constraint-concurrency")
	snapshot := inventoryPostgresValidSnapshot(tenantID, run, "inventorypg-concurrent-sku", "constraint-concurrency")
	...
	otherSnapshot := inventoryPostgresValidSnapshot(tenantID, run, "inventorypg-concurrent-manual", "constraint-manual")
	...
	manualErrors := runConcurrent(2, start, func(index int) error {
		req := inventoryPostgresManualRequest(tenantID, run, storedManual, store, string(rune('a'+index)))
		req.RequestID = "inventorypg-concurrent-manual-" + string(rune('a'+index))
		req.IdempotencyKeyHash = inventoryPostgresHash(req.RequestID)
	...
	base := inventoryPostgresValidRun(tenantID, store.ID, "same-run-concurrency")
	start = make(chan struct{})
	...
	replaySnapshot := inventoryPostgresValidSnapshot(tenantID, run, "inventorypg-concurrent-manual-replay", "constraint-manual-replay")
	...
	require.NoError(t, err)
	replayRequest := inventoryPostgresManualRequest(tenantID, run, storedReplay, store, "idempotent-replay")
	start = make(chan struct{})
...
func TestInventoryPostgresPageTransactionRollbackKeepsCursorAndStatistics(t *testing.T) {
	ctx := context.Background()
	db := newInventoryPostgresTestDB(t)
	tenantID := inventoryPostgresTenantID()
	store, _, _ := inventoryPostgresSeedShopAndSKU(t, db, tenantID, "page-rollback")

	callbackName := "inventorypg:fail_page_audit"
	db.Callback().Create().Before("gorm:create").Register(callbackName, func(tx *gorm.DB) {
		if tx.Statement != nil && tx.Statement.Schema != nil && tx.Statement.Schema.Table == "operation_logs" {
			if row, ok := tx.Statement.Dest.(*operationlog.OperationLog); ok && row.Action == "inventory_sync.page_processed" {
				tx.AddError(errors.New("inventorypg injected audit failure"))
			}
		}
	})
	...
	result, runErr := orchestrator.Run(ctx, InventorySyncOrchestratorInput{TenantID: tenantID, ShopConnectionID: store.ID, Platform: PlatformDouyin, ProviderMode: ProviderModeSandbox, FixtureScenario: FixtureScenarioSuccessSinglePage, ActorID: uuid.New(), RequestID: "inventorypg-page-rollback"})
...
func TestInventoryPostgresKeysetPaginationNoDuplicateOmissionAndScopeProtection(t *testing.T) {
	ctx := context.Background()
	db := newInventoryPostgresTestDB(t)
	tenantID := inventoryPostgresTenantID()
	store, _, _ := inventoryPostgresSeedShopAndSKU(t, db, tenantID, "keyset")
	repo := NewAPIRepository(db)
	at := time.Now().UTC().Truncate(time.Microsecond)

	for index := 0; index < 7; index++ {
		run := inventoryPostgresValidRun(tenantID, store.ID, string(rune('a'+index)))
		run.CreatedAt = at
		run.UpdatedAt = at
...
func TestInventoryPostgresBearerAuthAndFixtureGoldenPath(t *testing.T) {
	db := newInventoryPostgresTestDB(t)
	require.NoError(t, db.AutoMigrate(&inventory.InventoryChangeLog{}))
	tenantID := inventoryPostgresTenantID()
	store, _, _ := inventoryPostgresSeedShopAndSKU(t, db, tenantID, "auth-golden")
	actorID := uuid.New()
	...
	cfg := &config.Config{AppEnv: "test", JWTSecret: "inventory-postgres-integration-jwt-secret-with-safe-length", JWTExpHrs: 1}
	...
	request.Header.Set("Idempotency-Key", "inventorypg-auth-golden")
