...
var inventoryPostgresTenantCounter int64 = 790000

type inventoryPostgresAllowManualAuthorizer struct{}

func (inventoryPostgresAllowManualAuthorizer) CanResolveManualBinding(context.Context, int64, uuid.UUID, uuid.UUID) error {
	return nil
}
...
func inventoryPostgresTenantID() int64 {
	return atomic.AddInt64(&inventoryPostgresTenantCounter, 1)
}

func inventoryPostgresHash(value string) string {
	sum := sha256.Sum256([]byte(value))
	return hex.EncodeToString(sum[:])
}

func inventoryPostgresSeedShopAndSKU(t *testing.T, db *gorm.DB, tenantID int64, suffix string) (shop.Shop, product.Product, product.ProductSKU) {
	t.Helper()
	store := shop.Shop{
		TenantID:       tenantID,
		Platform:       PlatformDouyin,
		ShopName:       fmt.Sprintf("inventory-postgres-shop-%d-%s", tenantID, suffix),
		ExternalShopID: fmt.Sprintf("inventory-postgres-safe-shop-%d-%s", tenantID, suffix),
		Status:         "active",
		AuthStatus:     "mock",
	}
	require.NoError(t, db.Create(&store).Error)
	prod := product.Product{TenantID: tenantID, Source: "manual", Title: fmt.Sprintf("inventory-postgres-local-product-%d-%s", tenantID, suffix), Status: product.StatusDraft}
	require.NoError(t, db.Create(&prod).Error)
	sku := product.ProductSKU{ProductID: prod.ID, SKUCode: fmt.Sprintf("INVPG-SKU-%d-%s", tenantID, suffix), SKUName: "Inventory PostgreSQL Local SKU"}
	require.NoError(t, db.Create(&sku).Error)
	return store, prod, sku
}

func inventoryPostgresValidRun(tenantID int64, shopID uuid.UUID, suffix string) InventorySyncRun {
	now := time.Now().UTC()
	return InventorySyncRun{
		...
		RequestID:             fmt.Sprintf("inventorypg-run-%d-%s", tenantID, suffix),
		IdempotencyKeyHash:    inventoryPostgresHash("run-key-" + suffix),
		InputFingerprint:      inventoryPostgresHash("run-fingerprint-" + suffix),
		Revision:              1,
		StartedAt:             &now,
	}
}

func inventoryPostgresCreateRun(t *testing.T, ctx context.Context, db *gorm.DB, tenantID int64, shopID uuid.UUID, suffix string) *InventorySyncRun {
	t.Helper()
	run, err := NewInventorySyncRunRepository(db).Create(ctx, ptr(inventoryPostgresValidRun(tenantID, shopID, suffix)))
	require.NoError(t, err)
	return run
}

func inventoryPostgresValidSnapshot(tenantID int64, run *InventorySyncRun, externalSKUID string, suffix string) InventorySnapshotItem {
	return InventorySnapshotItem{
		TenantID:           tenantID,
		InventorySyncRunID: run.ID,
		ShopConnectionID:   run.ShopConnectionID,
		Platform:           run.Platform,
		ExternalProductID:  "inventorypg-remote-product-" + suffix,
		ExternalSKUID:      externalSKUID,
		ExternalSKUCode:    "INVPG-REMOTE-CODE-" + suffix,
		Barcode:            "INVPG-BARCODE-" + suffix,
		ProductTitle:       "Inventory PostgreSQL remote product",
		VariantTitle:       "safe variant",
		AvailableQuantity:  5,
		ReservedQuantity:   1,
		TotalQuantity:      8,
		ObservedAt:         time.Now().UTC(),
		PayloadHash:        inventoryPostgresHash("snapshot-payload-" + suffix),
		SafeMetadata:       datatypes.JSON([]byte(`{"quantityRelationshipContract":"provider_defined","fixture":"postgres"}`)),
	}
}

func inventoryPostgresCalibration(tenantID int64, run *InventorySyncRun, snapshot *InventorySnapshotItem, prod product.Product, sku product.ProductSKU, suffix string) SKUBindingCalibration {
	return SKUBindingCalibration{
		...
		InputFingerprint:        inventoryPostgresHash("calibration-input-" + suffix),
	}
}

func inventoryPostgresManualRequest(tenantID int64, run *InventorySyncRun, snapshot *InventorySnapshotItem, store shop.Shop, suffix string) *ManualBindingRequest {
	return &ManualBindingRequest{
		...
		CandidateCount:          2,
		RequestID:               fmt.Sprintf("inventorypg-manual-%d-%s", tenantID, suffix),
		IdempotencyKeyHash:      inventoryPostgresHash("manual-idempotency-" + suffix),
		InputFingerprint:        inventoryPostgresHash("manual-input-" + suffix),
		Revision:                1,
	}
}

func TestInventoryPostgresMigrationSchemaIndexesConstraintsAndJSONB(t *testing.T) {
	...
	for _, table := range []string{
		...
		require.Truef(t, db.Migrator().HasTable(table), "expected inventory sync table %s", table)
	}
...
func TestInventoryPostgresRepositoryConstraintsImmutabilityAndAtomicity(t *testing.T) {
	ctx := context.Background()
	db := newInventoryPostgresTestDB(t)
	tenantID := inventoryPostgresTenantID()
	suffix := fmt.Sprintf("repo-%d", tenantID)
	store, prod, sku := inventoryPostgresSeedShopAndSKU(t, db, tenantID, suffix)
	run := inventoryPostgresCreateRun(t, ctx, db, tenantID, store.ID, suffix)

	snapshotRepo := NewInventorySnapshotRepository(db)
	snapshot := inventoryPostgresValidSnapshot(tenantID, run, "inventorypg-remote-sku-"+suffix, suffix)
	require.NoError(t, snapshotRepo.CreateBatch(ctx, tenantID, []InventorySnapshotItem{snapshot}))
	require.ErrorIs(t, snapshotRepo.CreateBatch(ctx, tenantID, []InventorySnapshotItem{inventoryPostgresValidSnapshot(tenantID, run, snapshot.ExternalSKUID, suffix+"-duplicate")}), ErrDuplicateExternalSKU)
	storedSnapshot, err := snapshotRepo.GetByRunAndExternalSKU(ctx, tenantID, run.ID, snapshot.ExternalSKUID)
	require.NoError(t, err)

	negativeQuantity := inventoryPostgresValidSnapshot(tenantID, run, "inventorypg-negative-"+suffix, suffix+"-negative")
	...
	calibration := inventoryPostgresCalibration(tenantID, run, storedSnapshot, prod, sku, suffix)
	...
	badCalibration := inventoryPostgresCalibration(tenantID, run, storedSnapshot, prod, sku, suffix+"-bad")
...
func TestInventoryPostgresIdempotencyOptimisticConcurrencyAndManualResolution(t *testing.T) {
	ctx := context.Background()
	db := newInventoryPostgresTestDB(t)
	tenantID := inventoryPostgresTenantID()
	...
	first, err := runRepo.Create(ctx, ptr(inventoryPostgresValidRun(tenantID, store.ID, suffix)))
	require.NoError(t, err)
	second, err := runRepo.Create(ctx, ptr(inventoryPostgresValidRun(tenantID, store.ID, suffix)))
	require.NoError(t, err)
	require.Equal(t, first.ID, second.ID)
	conflictingRun := inventoryPostgresValidRun(tenantID, store.ID, suffix)
	conflictingRun.InputFingerprint = inventoryPostgresHash("different-fingerprint-" + suffix)
	...
	failedRun := inventoryPostgresValidRun(tenantID, store.ID, suffix+"-rerun-source")
	...
	activeRun := inventoryPostgresCreateRun(t, ctx, db, tenantID, store.ID, suffix+"-manual")
	snapshotRepo := NewInventorySnapshotRepository(db)
	snapshot := inventoryPostgresValidSnapshot(tenantID, activeRun, "inventorypg-manual-sku-"+suffix, suffix+"-manual")
	require.NoError(t, snapshotRepo.CreateBatch(ctx, tenantID, []InventorySnapshotItem{snapshot}))
	...
	manualRequest, err := manualRepo.Create(ctx, inventoryPostgresManualRequest(tenantID, activeRun, storedSnapshot, store, suffix))
	require.NoError(t, err)

	secondPending := *manualRequest
	secondPending.ID = uuid.Nil
	secondPending.RequestID = "inventorypg-manual-pending-duplicate-" + suffix
	secondPending.IdempotencyKeyHash = inventoryPostgresHash("manual-idempotency-duplicate-" + suffix)
	_, err = manualRepo.Create(ctx, &secondPending)
	require.ErrorIs(t, err, ErrManualBindingAlreadyPending)

	service := NewManualBindingService(db, inventoryPostgresAllowManualAuthorizer{})
	actorID := uuid.New()
	...
	for i := 0; i < 2; i++ {
		...
		go func(idx int) {
			...
			_, confirmErr := service.ConfirmBinding(ctx, ConfirmManualBindingInput{
				...
				CorrelationID:      fmt.Sprintf("inventorypg-confirm-%d-%s", idx, suffix),
				ExpectedRevision:   manualRequest.Revision,
				SelectedLocalSKUID: sku.ID,
				IdempotencyKeyHash: inventoryPostgresHash(fmt.Sprintf("manual-confirm-%d-%s", idx, suffix)),
				Comment:            "Inventory PostgreSQL manual confirmation",
			})
			errs <- confirmErr
		}(i)
	}
...
func TestInventoryPostgresManualBindingCrossTenantSelectedSKUIsolation(t *testing.T) {
	tenantA := inventoryPostgresTenantID()
	tenantB := inventoryPostgresTenantID()
	db := newInventoryPostgresTestDB(t)
	ctx := context.Background()
	storeA, _, _ := inventoryPostgresSeedShopAndSKU(t, db, tenantA, "manual-cross-a")
	_, _, skuB := inventoryPostgresSeedShopAndSKU(t, db, tenantB, "manual-cross-b")
	run := inventoryPostgresValidRun(tenantA, storeA.ID, "manual-cross")
	createdRun, err := NewInventorySyncRunRepository(db).Create(ctx, &run)
	require.NoError(t, err)
	snapshot := inventoryPostgresValidSnapshot(tenantA, createdRun, "inventorypg-manual-cross-sku", "manual-cross")
	...
	require.NoError(t, err)
	request, err := NewManualBindingRequestRepository(db).Create(ctx, inventoryPostgresManualRequest(tenantA, createdRun, stored, storeA, "manual-cross"))
	require.NoError(t, err)

	actorID := uuid.New()
	_, err = NewManualBindingService(db, inventoryPostgresAllowManualAuthorizer{}).ConfirmBinding(ctx, ConfirmManualBindingInput{
		Actor: ManualBindingActor{TenantID: tenantA, ActorID: actorID}, RequestID: request.ID,
		CorrelationID: "inventorypg-manual-cross", ExpectedRevision: request.Revision,
		SelectedLocalSKUID: skuB.ID, IdempotencyKeyHash: inventoryPostgresHash("manual-cross-confirm"),
	})
...
func TestInventoryPostgresAPIKeysetSafetyAndProductionBoundary(t *testing.T) {
	tenantID := inventoryPostgresTenantID()
	router, svc, actorID, shopID := newInventoryPostgresAPITestRouter(t, admin.RoleAdmin, tenantID)
	...
	router.ServeHTTP(rec, inventoryPostgresRequest(t, http.MethodPost, "/api/v1/inventory-sync/runs", payload, "inventorypg-create-"+fmt.Sprint(tenantID)))
	...
	router.ServeHTTP(prodCap, inventoryPostgresRequest(t, http.MethodPost, "/api/v1/inventory-sync/runs", map[string]any{"shopConnectionId": shopID, "platform": PlatformDouyin, "providerMode": "prod"}, "inventorypg-prod-cap-"+fmt.Sprint(tenantID)))
	require.Equal(t, http.StatusForbidden, prodCap.Code, prodCap.Body.String())

	readonly, _, _, readonlyShopID := newInventoryPostgresAPITestRouter(t, adminperm.RoleReadonly, inventoryPostgresTenantID())
	denied := httptest.NewRecorder()
	readonly.ServeHTTP(denied, inventoryPostgresRequest(t, http.MethodPost, "/api/v1/inventory-sync/runs", map[string]any{"shopConnectionId": readonlyShopID, "platform": PlatformDouyin, "providerMode": ProviderModeMock}, "inventorypg-readonly-"+fmt.Sprint(tenantID)))
...
func newInventoryPostgresAPITestRouter(t *testing.T, role string, tenantID int64) (*gin.Engine, *APIService, uuid.UUID, uuid.UUID) {
	t.Helper()
	db := newInventoryPostgresTestDB(t)
	require.NoError(t, db.AutoMigrate(&admin.AdminUser{}, &admin.UserStorePermission{}, &idempotency.Record{}, &operationlog.OperationLog{}))
	store, _, _ := inventoryPostgresSeedShopAndSKU(t, db, tenantID, "api")
	actorID := uuid.New()
	...
	r.Use(func(c *gin.Context) {
		...
		c.Set(ctxkey.TraceID, "trace-inventory-postgres-api-test")
		c.Next()
	})
...
func inventoryPostgresRequest(t *testing.T, method, requestPath string, body any, idem string) *http.Request {
	t.Helper()
	var raw []byte
	if body != nil {
