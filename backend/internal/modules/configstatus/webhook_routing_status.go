...
func (s *Service) webhookResolverItem(ctx context.Context) Item {
	it := Item{
...
func (s *Service) webhookAppBindingItem(_ context.Context) Item {
	return Item{
...
func (s *Service) webhookTenantIsolationItem(_ context.Context) Item {
	return Item{
...
func (s *Service) webhookProductionFallbackItem(_ context.Context) Item {
	it := Item{
...
func (s *Service) webhookConcurrencyItem(_ context.Context) Item {
	return Item{
...
func (s *Service) webhookRaceVerificationItem(_ context.Context) Item {
	return Item{
		Key:         "p32_linux_race_verification",
		Title:       "Linux Race 验证",
		SettingsURL: "/settings/config-status",
		Status:      StatusCodeReady,
		Summary:     "历史 race 记录已从工作树清理，可从 Git 历史查询；当前并发回归交由 GitHub Actions；不等于真实抖店 E2E 或 Production Ready",
	}
}
