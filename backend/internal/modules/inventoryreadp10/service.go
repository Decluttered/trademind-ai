...
type ManualReadService struct {
	DB          *gorm.DB
	Provider    inventorysyncp9.InventoryProvider
	Calibration *inventorysyncp9.SKUBindingCalibrationService
	Audit       *inventorysyncp9.InventorySyncAuditService
...
func NewManualReadService(db *gorm.DB, provider inventorysyncp9.InventoryProvider, catalog *metrics.Catalog, pageSize, maxPages int) *ManualReadService {
	policy, _ := inventorysyncp9.NewCalibrationThresholdPolicy(inventorysyncp9.CalibrationThresholdConfig{HighConfidenceThreshold: 9500, AutoConfirmationEnabled: false, PolicyVersion: inventorysyncp9.ThresholdPolicyVersionV1})
	...
	return &ManualReadService{DB: db, Provider: provider, Calibration: inventorysyncp9.NewSKUBindingCalibrationService(db, inventorysyncp9.NewGORMLocalSKUCandidateProvider(db), policy), Audit: inventorysyncp9.NewInventorySyncAuditService(db), Metrics: catalog, Environment: "unknown", PageSize: pageSize, MaxPages: maxPages}
}
...
func (s *ManualReadService) Run(ctx context.Context, input RunInput) (*RunResult, error) {
	...
	if input.SourceRunID != uuid.Nil {
		var source inventorysyncp9.InventorySyncRun
		...
		if source.Revision != input.ExpectedRevision || (source.Status != inventorysyncp9.InventorySyncRunStatusFailed && source.Status != inventorysyncp9.InventorySyncRunStatusCancelled) {
	...
	var existing inventorysyncp9.InventorySyncRun
	...
	started := s.now()
	run := inventorysyncp9.InventorySyncRun{TenantID: input.TenantID, ShopConnectionID: input.ShopID, Platform: inventorysyncp9.PlatformDouyin, ProviderMode: ProviderModeRealReadOnly, Status: inventorysyncp9.InventorySyncRunStatusRunning, Cursor: datatypes.JSON([]byte(`{}`)), Checkpoint: datatypes.JSON([]byte(`{"triggerType":"manual","readOnly":true}`)), SafeErrorMetadata: datatypes.JSON([]byte(`{}`)), RequestID: input.RequestID, IdempotencyKeyHash: input.IdempotencyHash, InputFingerprint: fingerprint, Revision: 1, StartedAt: &started}
	if input.SourceRunID != uuid.Nil {
	...
	s.Metrics.ObserveP10(s.Environment, "douyin", "inventory_read", "started", false, false, false, true, false, -1)
	_ = s.Audit.Write(ctx, inventorysyncp9.InventorySyncAuditEvent{TenantID: input.TenantID, ActorID: input.ActorID, Action: "inventory_sync.p10_read_started", Resource: "inventory_sync", ResourceID: run.ID.String(), ShopID: input.ShopID, Platform: inventorysyncp9.PlatformDouyin, Permission: adminperm.PermInventorySyncRun, Status: "success", RequestID: input.RequestID, Metadata: map[string]any{"providerMode": ProviderModeRealReadOnly, "readOnly": true, "automaticRetry": false}})
	pageSize := s.PageSize
	...
	for {
		...
		page, err := s.Provider.FetchInventoryPage(providerCtx, inventorysyncp9.InventoryFetchRequest{TenantID: input.TenantID, ShopConnectionID: input.ShopID.String(), Platform: inventorysyncp9.PlatformDouyin, ProviderMode: ProviderModeRealReadOnly, Cursor: cursor, PageSize: pageSize, MaxItemsPerPage: 100})
		if err != nil {
	...
	updates := map[string]any{"status": inventorysyncp9.InventorySyncRunStatusSucceeded, "cursor": cursor, "checkpoint": datatypes.JSON(checkpoint), "snapshot_count": snapshotCount, "calibration_count": calibrationCount, "manual_request_count": manualCount, "finished_at": finished, "revision": 2, "updated_at": finished}
	if err := s.DB.WithContext(ctx).Model(&inventorysyncp9.InventorySyncRun{}).Where("tenant_id = ? AND id = ? AND revision = 1", input.TenantID, run.ID).Updates(updates).Error; err != nil {
		return nil, err
	}
	run.Status, run.SnapshotCount, run.CalibrationCount, run.ManualRequestCount, run.Revision = inventorysyncp9.InventorySyncRunStatusSucceeded, snapshotCount, calibrationCount, manualCount, 2
	_ = s.Audit.Write(ctx, inventorysyncp9.InventorySyncAuditEvent{TenantID: input.TenantID, ActorID: input.ActorID, Action: "inventory_sync.p10_read_succeeded", Resource: "inventory_sync", ResourceID: run.ID.String(), ShopID: input.ShopID, Platform: inventorysyncp9.PlatformDouyin, Permission: adminperm.PermInventorySyncRun, Status: "success", RequestID: input.RequestID, Metadata: map[string]any{"snapshotCount": snapshotCount, "calibrationCount": calibrationCount, "manualBindingRequestCount": manualCount, "readOnly": true}})
	s.Metrics.ObserveInventory("douyin", "read_snapshot", "run", "success", "none", 1, finished.Sub(started))
	s.Metrics.ObserveP10(s.Environment, "douyin", "inventory_read", "success", false, false, false, false, false, manualCount)
	return runResult(run), nil
}

func (s *ManualReadService) failRun(ctx context.Context, run inventorysyncp9.InventorySyncRun, input RunInput, cause error, pages, snapshots int) (*RunResult, error) {
	finished := s.now()
	...
	_ = s.DB.WithContext(ctx).Model(&inventorysyncp9.InventorySyncRun{}).Where("tenant_id = ? AND id = ?", input.TenantID, run.ID).Updates(map[string]any{"status": inventorysyncp9.InventorySyncRunStatusFailed, "safe_error_metadata": datatypes.JSON(meta), "finished_at": finished, "revision": 2, "updated_at": finished}).Error
	_ = s.Audit.Write(ctx, inventorysyncp9.InventorySyncAuditEvent{TenantID: input.TenantID, ActorID: input.ActorID, Action: "inventory_sync.p10_read_failed", Resource: "inventory_sync", ResourceID: run.ID.String(), ShopID: input.ShopID, Platform: inventorysyncp9.PlatformDouyin, Permission: adminperm.PermInventorySyncRun, Status: "failed", RequestID: input.RequestID, Metadata: map[string]any{"errorCode": code, "safeMessage": code, "readOnly": true, "automaticRetry": false}})
	s.Metrics.ObserveInventory("douyin", "read_snapshot", "run", "failure", code, 1, finished.Sub(*run.StartedAt))
	s.Metrics.ObserveP10(s.Environment, "douyin", "inventory_read", code, true, false, false, false, true, -1)
	return nil, cause
}

func makeSnapshot(run inventorysyncp9.InventorySyncRun, item inventorysyncp9.InventoryProviderItem, observed time.Time) (inventorysyncp9.InventorySnapshotItem, error) {
	payload, err := json.Marshal(item)
	if err != nil {
		return inventorysyncp9.InventorySnapshotItem{}, err
	}
...
func runResult(run inventorysyncp9.InventorySyncRun) *RunResult {
