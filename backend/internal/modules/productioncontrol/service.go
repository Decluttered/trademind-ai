...
func (s *Service) Status(ctx context.Context, tenantID int64, allowedShopIDs []uuid.UUID) (*RuntimeStatus, error) {
	...
	if strings.TrimSpace(s.Config.ProductionCapabilities.DouyinAPIBaseURL) != "" {
	...
	return &RuntimeStatus{
		CurrentAllowedLevel: s.Config.ProductionCapabilities.CurrentAllowedLevel, Environment: s.Config.AppEnv,
		...
		RealProviderEnabled:           s.Config.ProductionCapabilities.RealProviderEnabled, RealPlatformNetworkEnabled: s.Config.ProductionCapabilities.RealPlatformNetworkEnabled, RealCredentialsEnabled: s.Config.ProductionCapabilities.RealCredentialsEnabled, RealInventoryReadEnabled: s.Config.ProductionCapabilities.RealInventoryReadEnabled,
		RealProductDraftWriteEnabled: s.Config.ProductionCapabilities.RealProductDraftWriteEnabled,
		RealInventoryWriteEnabled:    false, InventoryMutationEnabled: s.Config.ProductionCapabilities.InventoryMutationEnabled, BackgroundWorkerEnabled: s.Config.ProductionCapabilities.BackgroundWorkerEnabled, ProductPublishQueueEnabled: s.Config.ProductPublishQueueEnabled, ProviderWriteReady: providerWriteReady, AutomaticRetryEnabled: s.Config.ProductionCapabilities.AutomaticRetryEnabled, ReadOnlyCapability: !s.Config.ProductionCapabilities.RealProductDraftWriteEnabled,
		OfflineOAuthEnabled: s.Config.ProductionCapabilities.OfflineOAuthEnabled, OfflineCredentialAvailable: s.Config.ProductionCapabilities.OfflineOAuthEnabled && strings.TrimSpace(s.Config.ProductionCapabilities.LocalCredentialKey) != "",
...
func (s *Service) EvaluateRead(ctx context.Context, tenantID int64, shopID uuid.UUID, skuCount int) error {
	...
	if s.Config.ProductionCapabilities.CurrentAllowedLevel == "L0" || !s.Config.ProductionCapabilities.RealProviderEnabled || !s.Config.ProductionCapabilities.RealPlatformNetworkEnabled || !s.Config.ProductionCapabilities.RealCredentialsEnabled || !s.Config.ProductionCapabilities.RealInventoryReadEnabled {
		return ErrBlocked
	}
	var allow ScopeAllowlist
...
func (s *Service) EvaluateWrite(ctx context.Context, tenantID int64, shopID, productID uuid.UUID, skuCount int) error {
	...
	capabilities := s.Config.ProductionCapabilities
	if !strings.EqualFold(capabilities.CurrentAllowedLevel, "L3") || !capabilities.RealProviderEnabled || !capabilities.RealPlatformNetworkEnabled || !capabilities.RealCredentialsEnabled || !capabilities.RealProductDraftWriteEnabled || !capabilities.BackgroundWorkerEnabled || !s.Config.ProductPublishQueueEnabled || capabilities.AutomaticRetryEnabled {
		return ErrBlocked
	}
...
func writeReady(cfg *config.Config, control RuntimeControl, allow *ScopeAllowlist, gray *GrayPolicy, providerWriteReady bool) bool {
	...
	capabilities := cfg.ProductionCapabilities
	return strings.EqualFold(capabilities.CurrentAllowedLevel, "L3") && capabilities.RealProviderEnabled && capabilities.RealPlatformNetworkEnabled && capabilities.RealCredentialsEnabled && capabilities.RealProductDraftWriteEnabled && capabilities.BackgroundWorkerEnabled && cfg.ProductPublishQueueEnabled && !capabilities.AutomaticRetryEnabled && providerWriteReady &&
