...
func TestGrayApprovalRequiresTwoDifferentAdminsBeforeActivation(t *testing.T) {
	db := openControlTestDB(t)
	svc := &Service{DB: db, Config: &config.Config{ProductionCapabilities: enabledDraftWriteConfig(), ProductPublishQueueEnabled: true}, ProviderWriteGuard: allowProviderWrite}
	ctx := context.Background()
	tenantID := int64(101)
	shopID := uuid.New()
...
func TestProviderWriteGuardFailsClosedForEvaluationAndStatus(t *testing.T) {
	...
	svc := &Service{
		DB:     db,
		Config: &config.Config{AppEnv: "production", ProductionCapabilities: enabledDraftWriteConfig(), ProductPublishQueueEnabled: true},
...
func enabledDraftWriteConfig() config.ProductionCapabilityConfig {
	return config.ProductionCapabilityConfig{
