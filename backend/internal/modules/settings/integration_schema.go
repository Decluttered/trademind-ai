...
func IntegrationConfigDefinitions() []IntegrationConfigSchema {
	return []IntegrationConfigSchema{
		...
		},
		{
			...
			Title:       "告警外部通知",
			Category:    "ops",
			Description: "任务告警支持邮件、通用 Webhook、飞书群机器人和企业微信群机器人。Webhook URL / Secret 与机器人地址加密保存；SMTP 使用「邮件设置」。详见管理端「告警通知」页。",
			...
			Fields: []IntegrationFieldSchema{
				...
				{Name: "feishu_enabled", Label: "飞书群机器人", Type: "switch", Required: false},
				{Name: "feishu_webhook_url", Label: "飞书机器人回调通知地址", Type: "password", Required: false, Sensitive: true},
				{Name: "feishu_secret", Label: "飞书签名 Secret", Type: "password", Required: false, Sensitive: true},
				{Name: "wecom_enabled", Label: "企业微信群机器人", Type: "switch", Required: false},
				{Name: "wecom_webhook_url", Label: "企业微信机器人回调通知地址", Type: "password", Required: false, Sensitive: true},
			},
		},
		{
			Key:         "collect_rules",
