...
旧版 `p9_*` / `p10_*` 表、历史图像任务子表 `ai_image_task_items` 以及带 `p7` 阶段号的性能索引，会在 `AutoMigrate` 创建当前模型前执行事务内原地重命名；图像任务子表的当前名称为 `image_task_items`。数据、主键和外键关系保持不变，关联索引、约束、触发器及不可变保护函数同步改名。如果旧名和新名同时存在，迁移会拒绝启动，禁止自动合并或覆盖数据。
