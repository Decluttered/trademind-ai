...
| Collector Provider | `collector/`（当前仅 `sourceAmazon`）、`backend/internal/modules/collect`、`modules/catalog`、采集任务 API、队列、raw 原始数据、`COLLECTOR_SERVICE_TOKEN`、HTTP 请求限制、URL/DNS/WebSocket 出站保护、Docker 内部网络、`docs/provider.md`、`docs/amazon-to-ebay.md`、[ADR-0003](adr/0003-amazon-source-collector.md) |
| MindBay Companion Extension | `extension/`、独立 Extension Token Audience、Go Extension API、`docs/adr/0004-wxt-companion-extension.md`、`docs/mindbay-module-matrix.md`、`tests/architecture/module-boundaries.json`；禁止直接 DB/eBay 调用或在 Content Script 保存平台 Secret。 |
| MindBay Discovery / Listing Studio | `backend/internal/modules/catalog`、`listingstudio`、`listingcalc`、`extensiontoken`、`collector/src/providers/sourceAmazon`、`admin/src/pages/MindBay`、`admin/src/services/mindbay.ts`、`docs/api.md`、Contract-/Architekturtests；检查 workspace scope、Cents、Snapshot/Content-Version-Immutabilität、GPSR、Idempotenz und SSRF-Schutz。 |
| MindBay Kalender / eBay Publish | `backend/internal/modules/publication`、`backend/internal/providers/platform/ebay`、`backend/internal/workflow/temporal`、`temporal/`、`modules/shop` eBay OAuth/Taxonomy、`admin/src/pages/MindBay/Planner`、`docs/runbooks/MINDBAY_EBAY_PUBLICATION.md`；检查单一 Temporal Writer、稳定 SKU、Offer Reconcile、DRY_RUN、Sandbox/Production Guard、GPSR、加密 Token、Slot/Job Idempotenz und Douyin-Regression。 |
| MindBay Monitoring / Repricing / Profit | `backend/internal/modules/monitoring`、`publication.ListingSnapshot`、`providers/platform/ebay` Offer Read/Update、`admin/src/pages/MindBay/{Monitoring,Profit}`、`admin/src/services/mindbay.ts`、`docs/api.md`；检查 Cents、immutable Regelversionen/Snapshots、Decision-Idempotenz、Cooldown/Margen-Gates、DRY_RUN、Offer-Verify und expected/actual Ledger-Trennung。 |
